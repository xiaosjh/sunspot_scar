...
cd,hmi_dirs[idir]
print,'Processing: ',hmi_dirs[idir]

field_files = file_search(hmi_dirs[idir] + '\*field.fits', count=n_field)
incl_files  = file_search(hmi_dirs[idir] + '\*inclination.fits', count=n_incl)
azim_files  = file_search(hmi_dirs[idir] + '\*azimuth.fits', count=n_azim)
disam_files = file_search(hmi_dirs[idir] + '\*disambig.fits', count=n_disam)

if (n_field eq 0) or (n_incl eq 0) or (n_azim eq 0) or (n_disam eq 0) then begin
  print,'  Missing one or more HMI B files, skip.'
  continue
endif
...
write_png,'bxyz.png',tvrd()
;注释掉的这一段是裁减成一个submap了
;sub_map,mapbz,smapbz,xrange=[500,800.0],yrange=[-500,-100.0]
...
;i=0
save,mapbx,mapby,mapbz,filename=hmi_dirs[idir] + '\bxyz.sav'
